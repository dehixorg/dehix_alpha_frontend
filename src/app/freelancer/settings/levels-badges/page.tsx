    ...
    >
      ...
      <Card className="mb-8">
        <CardHeader className="space-y-1">
          <CardTitle className="text-base">How levels & badges work</CardTitle>
          <CardDescription>
            Every 3 levels form a stage (Beginner → Intermediate → ...). Within
            each stage the tiers go Bronze → Silver → Gold.
          </CardDescription>
        </CardHeader>
        <CardContent>
          <div className="grid gap-3 sm:grid-cols-3">
            {[getStepTierMeta(0), getStepTierMeta(1), getStepTierMeta(2)].map(
              (tier, idx) => {
                if (!tier) return null;
        ...
        <Card>
          <CardHeader className="flex-row items-center justify-between space-y-0">
            ...
            <div className="flex items-center gap-2">
              ...
              />
              <label htmlFor="show-eligible" className="text-sm font-medium">
                Eligible only
              </label>
            </div>
          </CardHeader>
