...
export function subscribeToUserConversations(
  ...
  activeUnsubscribe = onSnapshot(filteredQuery, processSnapshot, (error) => {
  ...
  try {
    await runTransaction(db, async (transaction) => {
      ...
      if (senderId && typeof senderId === 'string') {
        ...
        logger.warn(
          'Skipping unread count update: senderId is missing or invalid',
          {
            conversationId,
            senderId,
          },
      ...
      transaction.update(
        conversationRef,
        conversationUpdate as Record<string, any>,
      );
...
export async function updateDataInFirestore(
  ...
  try {
    ...
    logger.error('Error updating document:', (error as FirestoreError).message);
    throw error; // Re-throw so caller knows it failed
  }
}
...
export async function loadMoreMessages(
  ...
  while (allMessages.length < pageSize && hasMore) {
    ...
    const batchSnapshot: QuerySnapshot<DocumentData> =
  ...
  try {
    await runTransaction(db, async (transaction) => {
      ...
      if (
        ...
        throw new Error(
          'Unauthorized: Only participants or admins can pin messages',
        );
      }
  ...
  try {
    await runTransaction(db, async (transaction) => {
      ...
      if (
        ...
        throw new Error(
          'Unauthorized: Only participants or admins can unpin messages',
        );
      }
