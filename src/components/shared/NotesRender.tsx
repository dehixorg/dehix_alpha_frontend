...
const NotesRender = ({ notes, setNotes, isArchive, isTrash, fetchNotes }: NotesRenderProps) => {
    ...
    const handleNoteUpdate = async (noteId: string, noteType: string) => {
        ...
        try {
            const response = await axiosInstance.put(`/notes/update/${noteToUpdate._id}`, {
                ...noteToUpdate,
                noteType,
            });
    ...
    return (
        <div className="flex justify-center items-center">
                                ...
                                <CardHeader>
                                    {note.type && (
                                        <div className="absolute top-1 left-1">
                                            <Badge className={`text-xs py-0.5 ${badgeColors[note.type] || " "}`}>{note.type}</Badge>
                                        </div>
                                    )}
                                    {note.title && (
                                        <CardTitle className="font-semibold text-lg text-black mt-6">{note.title}</CardTitle>
                                    )}
                                </CardHeader>
                            ...
                            <div className="relative">
                                <div className="absolute bottom-2 right-2 hidden group-hover:flex items-center gap-4 justify-center">
                                    ...
                                    <BannerChangerPopover
                                        handleChangeBanner={(banner) => handleChangeBanner(note._id, banner)}
                                    />
                                    {!isTrash && (
                                        <DropdownNavNotes
                                            noteId={note._id}
            ...
            {isDeleting && <DialogConfirmation onClose={handleDialogClose} note={selectedDeleteNote} onDelete={handleDeletePermanently} />}
            {selectedNote && <DialogSelectedNote onSave={handleSaveEditNote} note={selectedNote} onClose={handleDialogClose} />}
            {selectedTypeNote && (
                <DialogUpdateType
