...
export function ChatList({
  ...
  const handleProfileIconClick = (e: React.MouseEvent, conv: Conversation) => {
    ...
    if (conv.type === 'group') {
      ...
      const otherParticipantUid = conv.participants.find(p => p !== user.id);
  ...
  const handleStartNewChat = async (selectedUser: NewChatUser) => {
    ...
    const existingConversation = conversations.find(conv =>
      ...
      arraysHaveSameElements(conv.participants, [user.id, selectedUser.id])
    );
    ...
    const newConversationData: Partial<Conversation> = { // Use Partial as ID will be generated by Firestore
      ...
      lastMessage: undefined, // Changed from null to undefined to match the type
      participantDetails: {
        [user.id]: {
        ...
        [selectedUser.id]: { // Use .id from CombinedUser
          userName: selectedUser.displayName, // Use displayName from CombinedUser
    ...
    try {
      ...
      setIsNewChatDialogOpen(false);
      toast({ title: "Success", description: `New chat started with ${selectedUser.displayName}.` }); // Use displayName
                      ...
                      <AvatarImage
                        src={
                          conversation.type === 'group'
                            ...
                            : conversation.participantDetails?.[conversation.participants.find(p => p !== user.id) || '']?.profilePic || `https://api.adorable.io/avatars/285/${conversation.participants.find(p => p !== user.id)}.png` // User avatar
                        }
                        alt={conversation.type === 'group' ? conversation.groupName : conversation.participantDetails?.[conversation.participants.find(p => p !== user.id) || '']?.userName}
                      />
                      <AvatarFallback>
                        {
                          (conversation.type === 'group'
                            ? conversation.groupName?.charAt(0)
                            : conversation.participantDetails?.[conversation.participants.find(p => p !== user.id) || '']?.userName?.charAt(0)
                          )?.toUpperCase() || 'P'
                        }
                      </AvatarFallback>
                    </Avatar>
                  ...
                  <div className="flex-grow overflow-hidden" onClick={() => setConversation(conversation)}> {/* Make text area also set active conversation */}
                    <div className="flex justify-between items-baseline">
                      <p className={cn("text-sm font-medium truncate", isActive ? "text-[hsl(var(--primary))]" : "text-[hsl(var(--foreground))]")}>
                        {conversation.type === 'group' ? conversation.groupName : conversation.participantDetails?.[conversation.participants.find(p => p !== user.id) || '']?.userName || 'Chat User'}
                      </p>
                    ...
                    {groupName.split(',').map(name => (
                      ...
                      >
                        ...
                        <button
                          ...
                          onClick={(e) => {
                            e.stopPropagation();
                            const newGroupName = groupName.replace(name, '').trim();
            ...
            <DialogFooter className="flex justify-end space-x-2 pt-4 border-t border-[hsl(var(--border))]"> {/* Applied flex, justify-end, space-x-2 and adjusted padding */}
              ...
              <Button
                ...
                onClick={async () => {
                  ...
                  const participantUIDs = Array.from(new Set([currentUserUID, ...groupName.split(',').map(name => name.trim())]));
