...
export const SkillInfo: React.FC<SkillInfoProps> = ({
  ...
  const [options, setOptions] = useState<Array<{ _id: string; name: string; label: string; skillName: string }>>([]);
  ...
  const handleRemoveByName = (name: string) => {
    ...
    if (skill) {
      onRemoveSkill(skill._id || skill.name || skill.skillName || skill.label || '');
    }
  };
