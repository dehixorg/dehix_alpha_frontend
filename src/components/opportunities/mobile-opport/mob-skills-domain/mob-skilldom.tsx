...
const MobileSkillDom: React.FC<MobileSkillDomProps> = ({ heading, checkboxLabels }) => {
            ...
            <div className="items-center p-2">
                ...
                {filteredSkills.length > 3 && (
                    <div className="flex items-center mb-1">
                        <button className="text-sm text-blue-500 cursor-pointer" onClick={() => setShowMore(!showMore)}>
                    ...
                    hiddenSkills.map((label, index) => (
                        <div key={index} className="flex items-center mb-1">
                            <input
                                ...
                                onChange={() => handleCheckboxChange(visibleSkills.length + index)}
                                className="mr-2"
                            />
                            <label className="text-sm text-black">{label}</label>
                        </div>
