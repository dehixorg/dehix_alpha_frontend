...
export default function ReviewBidsDetail({ interviewId, onBack }: ReviewBidsDetailProps) {
  ...
  const handleAcceptBid = async (bidId: string) => {
    try {
      ...
      await axiosInstance.post(`/interview/${interviewId}/interview-bids/${bidId}`);
      ...
      notifyError(error.response?.data?.message || 'Failed to hire candidate', 'Error');
    } finally {
  ...
  const handleRejectBid = async (bidId: string) => {
    try {
      setProcessingId(bidId);
      await axiosInstance.put(`/interview/${interviewId}/interview-bids/${bidId}`, {
        status: 'REJECTED'
      });
      notifySuccess('Bid rejected', 'Status Updated');
      fetchInterview(); // Refresh the list
    } catch (error: any) {
      notifyError(error.response?.data?.message || 'Failed to reject bid', 'Error');
    } finally {
      setProcessingId(null);
    }
  };
    ...
    return (
      <div className="space-y-8 animate-in fade-in duration-500">
        ...
        <div className="grid gap-6 md:grid-cols-2">
          {[1, 2].map((i) => (
            <Card key={i} className="border-muted/40 bg-card/40 backdrop-blur-sm">
  ...
  if (!interview) {
    return (
      <div className="flex flex-col items-center justify-center py-20 text-center">
        ...
        <p className="text-muted-foreground mt-2">The requested interview details could not be found.</p>
    ...
    <div className="space-y-10 animate-in fade-in slide-in-from-bottom-4 duration-700">
        ...
        <div className="space-y-4">
          ...
          <div className="space-y-2">
            <div className="flex items-center gap-3">
              ...
              <Badge variant="outline" className="bg-primary/5 text-primary border-primary/20 h-6 px-3 font-bold uppercase tracking-widest text-[9px]">
                {interview.interviewType}
              </Badge>
            </div>
            <p className="text-lg text-muted-foreground max-w-3xl leading-relaxed">
              {interview.description || 'Review candidates and select the best expert for your interview needs.'}
            </p>
          </div>
        </div>
        ...
        <Card className="border-muted/40 bg-card/30 backdrop-blur-md shadow-lg rounded-2xl lg:w-80">
          <CardContent className="p-5 space-y-4">
            <div className="flex items-center gap-3 text-sm font-medium">
               ...
               <span className="font-bold">{interview.interviewDate ? new Date(interview.interviewDate).toLocaleDateString() : 'N/A'}</span>
            </div>
            <div className="flex items-center gap-3 text-sm font-medium">
               ...
               <span className="font-bold">{interview.interviewDate ? new Date(interview.interviewDate).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }) : 'N/A'}</span>
            </div>
      ...
      <div className="space-y-8">
        <div className="flex items-center gap-4">
          ...
          <h2 className="text-2xl font-bold tracking-tight">Active Candidates</h2>
        </div>
        ...
        {pendingBids.length === 0 ? (
          <div className="py-12 border-2 border-dashed border-muted/50 rounded-3xl bg-muted/5 flex flex-col items-center justify-center text-center px-6">
            <p className="text-lg font-medium text-muted-foreground">No pending candidates to review.</p>
            <p className="text-sm text-muted-foreground/60 mt-1">New bids will appear here as soon as interviewers respond.</p>
          </div>
        ) : (
          <div className="grid gap-6 md:grid-cols-2">
            {pendingBids.map((bid) => (
        ...
        {otherBids.length > 0 && (
          <div className="space-y-6 pt-10">
            <div className="flex items-center gap-4 opacity-70">
              ...
              <h2 className="text-xl font-bold tracking-tight text-muted-foreground">Processed</h2>
            </div>
...
function CandidateCard({ 
  ...
  const fullName = `${interviewer?.firstName || ''} ${interviewer?.lastName || ''}`.trim() || interviewer?.userName || 'Anonymous Expert';
          ...
          <div className="flex-1 space-y-1 min-w-0">
            ...
            <div className="flex flex-wrap gap-2 pt-1">
              {(interviewer?.skills || []).slice(0, 3).map((skill, idx) => (
                <span key={idx} className="text-[10px] font-bold text-muted-foreground/70 bg-muted/40 px-2 py-0.5 rounded-md">
                  {skill}
                </span>
              ))}
        ...
        <div className="space-y-3">
          <div className="flex items-center justify-between">
            ...
            <div className="text-lg font-black text-foreground">
               ${bid.fee}
               <span className="text-xs font-bold text-muted-foreground/60 ml-1">Total Fee</span>
            </div>
          </div>
          <div className="p-4 rounded-2xl bg-muted/20 border border-muted/40 text-sm leading-relaxed text-muted-foreground group-hover:text-foreground transition-colors italic">
            &quot;{bid.description || 'No specific description provided for this bid.'}&quot;
          </div>
        </div>

        <div className="grid grid-cols-2 gap-4">
           <div className="space-y-1">
              <p className="text-[10px] font-bold uppercase tracking-tighter text-muted-foreground/50">Experience</p>
           ...
           <div className="space-y-1 text-right">
              <p className="text-[10px] font-bold uppercase tracking-tighter text-muted-foreground/50">Rating</p>
              <div className="flex items-center justify-end gap-1 text-sm font-bold">
                 <span>4.9</span>
                 <div className="flex">
                    {[1,2,3,4,5].map(i => <Star key={i} className="h-2 w-2 text-amber-500 fill-amber-500" />)}
                 </div>
              </div>
           </div>
        </div>
