...
import { Dehix_Talent_Card_Pagination, type HireDehixTalentStatusEnum } from "@/utils/enum"
...
const TalentCard: React.FC<TalentCardProps> = ({ skillFilter, domainFilter, skillDomainFormProps }) => {
  ...
  const fetchUserData = useCallback(async () => {
    try {
      ...
      if (user?.uid) {
        ...
        const formattedHireTalentData = Object.values(hireTalentData).map((item: any) => ({
          uid: item._id,
        ...
        if (skillsResponse?.data?.data) {
          const uniqueSkills = skillsResponse.data.data.filter(
            (skill: any) => !filterSkills.some((filterSkill: any) => filterSkill._id === skill._id),
          )
        ...
        if (domainsResponse?.data?.data) {
          const uniqueDomain = domainsResponse.data.data.filter(
            (domain: any) => !filterDomains.some((filterDomain: any) => filterDomain._id === domain._id),
          )
  ...
  }, [user?.uid, skillDomainFormProps?.skillFilter, skillDomainFormProps?.domainFilter])
  ...
  const handleDeleteSkill = (skillToDelete: string) => {
    setCurrSkills(currSkills.filter((skill: any) => skill.name !== skillToDelete))
  }
    ...
    async (newSkip = skip, reset = false) => {
      ...
      try {
        ...
        if (response.data.data.length < Dehix_Talent_Card_Pagination.BATCH) {
          setHasMore(false)
          setTalents((prev) => (reset ? response.data.data : [...prev, ...response.data.data]))
          return
        }
        ...
        return (
          <Card key={talentEntry._id} className="w-full sm:w-[350px] lg:w-[450px]">
            <CardHeader className="flex flex-row items-center gap-4">
              ...
              <button onClick={() => toggleBookmark(talentEntry._id)} className="ml-auto">
                <Star
                    ...
                    <Sheet key={View}>
                      ...
                      <SheetContent side={View} className="overflow-y-auto no-scrollbar max-h-[100vh]">
                        ...
                        <table className="min-w-full table-auto border-collapse ">
                          <tbody>
                            <tr>
                              <td className="border-b px-4 py-2 font-medium">Username</td>
                              <td className="border-b px-4 py-2">{talent.userName || "N/A"}</td>
                            </tr>
                            <tr>
                              <td className="border-b px-4 py-2 font-medium">Skill</td>
                              <td className="border-b px-4 py-2">{talentEntry.skillName || "N/A"}</td>
                            </tr>
                            <tr>
                              <td className="border-b px-4 py-2 font-medium">Domain</td>
                              <td className="border-b px-4 py-2">{talentEntry.domainName || "N/A"}</td>
                            </tr>
                            <tr>
                              <td className="border-b px-4 py-2 font-medium">Experience</td>
                              <td className="border-b px-4 py-2">{talentEntry.experience} years</td>
                            </tr>
                            <tr>
                              <td className="border-b px-4 py-2 font-medium">Monthly Pay</td>
                              <td className="border-b px-4 py-2">${talentEntry.monthlyPay}</td>
                            </tr>
                            <tr>
                              <td className="border-b px-4 py-2 font-medium">Github</td>
                              <td className="border-b px-4 py-2">{talent.Github || "N/A"}</td>
                            </tr>
                            <tr>
                              <td className="border-b px-4 py-2 font-medium">LinkedIn</td>
                              <td className="border-b px-4 py-2">{talent.LinkedIn || "N/A"}</td>
                            </tr>
                          </tbody>
                        </table>
                        ...
                        <div className="w-full mt-4 mb-6">
                          <div className="w-full">
                            <div className="flex items-center mt-2">
                              <Select onValueChange={(value) => setTmpSkill(value)} value={tmpSkill || ""}>
                                <SelectTrigger>
                          ...
                          <div className="mt-2">
                            <div className="w-full text-center">
                              <Button className="w-full text-sm py-1 px-2  text-black rounded-md" type="submit">
                                Save
                              </Button>
                            </div>
                          </div>
